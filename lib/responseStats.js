...
class ResponseStats {
  ...
  listen() {
    this.eventEmitter.on('aietes-stats', (data) => {
      ...
      this.stats[path][method].queryParams.push({ ...queryParams }) // TODO merge these 3 into one object
      this.stats[path][method].headers.push({ ...headers })
    })
  }
  ...
  headers(path, method) {
    ...
    return [...this.stats[path][method].headers]
  }
}
